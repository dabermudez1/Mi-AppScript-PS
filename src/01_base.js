...
class PatientRepository {
  ...
  findAll() {
    ...
    return data.slice(1).map((row, i) => {
      const obj = { _row: i + 2 };
      Object.keys(idx).forEach(key => obj[key] = row[idx[key]]);
      return obj;
    });
  }
...
class SessionRepository {
  ...
  insertAll(sessions) {
    ...
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(this.sheetName);
    const headers = HEADERS[this.sheetName];
    const data = sessions.map(s => headers.map(h => s[h] || ''));
    sheet.getRange(sheet.getLastRow() + 1, 1, data.length, headers.length).setValues(data);
  }
  save(s) { /* Implementación similar a PatientRepo si es necesario */ }
  saveAll(sessions) { /* Implementación para actualización masiva */ }
  findPendientesByPaciente(pacienteId) {
    return this.findAll().filter(s => String(s.PacienteID) === String(pacienteId) && s.EstadoSesion === 'PENDIENTE');
  }
}

class ConfigRepository {
  constructor() { this.sheetName = SHEET_CONFIG_MODALIDADES; }
  findAll() {
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(this.sheetName);
    const data = sheet.getDataRange().getValues();
    const idx = indexByHeader_(data[0]);
  ...
  findByModalidad(mod) {
    return this.findAll().find(c => c.Modalidad === mod);
  }
}
